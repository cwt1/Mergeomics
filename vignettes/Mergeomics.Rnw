...
Mergeomics\textunderscore 0.99.1.tar.gz 
...
Here, we first provide an one-step way to run the whole pipeline. Then, the 
step-by-step processing will be described in detail in separate sections.
...
Part of the original result is listed in Table 1. The pathways 
...
could merge the disease-related pathways into relatively independent gene sets
after running MSEA.
